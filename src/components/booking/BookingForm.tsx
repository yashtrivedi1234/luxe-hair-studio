...
export function BookingForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success("Booking received! We'll confirm on WhatsApp/email.");
    } catch (err) {
  ...
  if (done) {
    ...
    return (
      <div className="rounded-2xl border bg-card p-8 text-center shadow-soft space-y-4">
        <h2 className="font-display text-3xl font-semibold text-primary">Booking Received</h2>
        <p className="text-muted-foreground">
          Ref: {done.id.slice(0, 8).toUpperCase()} — {svc?.name} on {date} at {time}
        </p>
        <p className="text-sm text-muted-foreground">
          Our team will confirm shortly by call/WhatsApp
          {email ? " and email" : ""}.
        </p>
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="rounded-2xl border border-border/70 bg-card/90 backdrop-blur p-6 md:p-9 shadow-card space-y-5"
    >
      <div>
